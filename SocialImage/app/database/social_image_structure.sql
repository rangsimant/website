...
CREATE TABLE `account` (
  ...
  `account_subject` varchar(45) DEFAULT 'samsung',
  `account_available` enum('open','close') NOT NULL DEFAULT 'open',
  PRIMARY KEY (`account_id`),
  UNIQUE KEY `account_id_user_UNIQUE` (`account_id_user`),
  UNIQUE KEY `account_username_UNIQUE` (`account_username`,`account_channel`),
  KEY `account_subject_fk_idx` (`account_subject`),
  CONSTRAINT `account_subject_fk` FOREIGN KEY (`account_subject`) REFERENCES `subject` (`subject_name`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE `post` (
  ...
  `post_img_name` varchar(100) DEFAULT NULL,
  `post_link` text,
  `post_subject` varchar(45) DEFAULT 'samsung',
  `post_available` enum('open','close') NOT NULL DEFAULT 'open',
  `post_tab` varchar(45) DEFAULT 'all',
  `post_url_image` varchar(600) DEFAULT NULL,
  `postcol` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`post_id`),
  UNIQUE KEY `post_social_id_UNIQUE` (`post_social_id`),
  KEY `author_id_fk_idx` (`author_id`),
  KEY `post_subject_fk_idx` (`post_subject`),
  CONSTRAINT `author_id_fk` FOREIGN KEY (`author_id`) REFERENCES `author` (`author_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `post_subject_fk` FOREIGN KEY (`post_subject`) REFERENCES `subject` (`subject_name`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=540871 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2015-08-07  1:54:45
